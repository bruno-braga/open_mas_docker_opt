// beliefs and rules
kqml::bel_no_source_self(NS::Content,Ans) :- (NS::Content[|LA] & (kqml::clear_source_self(LA,NLA) & ((Content =.. [F,T,_648]) & (Ans =.. [NS,F,T,NLA])))).
...
my_testing(0.23815524453110348).
...
focused(WksName,ArtName[artifact_type(Type)],ArtId) :- focusing(ArtId,ArtName,Type,_606,WksName,_607).
...
joined(WksName,WksId) :- joinedWsp(WksId,WksName,_608).
last_dir(down).
...
@p__105[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art([],_603).
...
@p__107[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art(art_env(W,"",_604),Try) <- .concat("/main/",W,FullW); joinWorkspace(FullW,_605); .print("joinned workspace ",FullW).
...
@pfunction2[atomic,source(self),url("file:src/agt/default_agent.asl")] +initial_print1 <- .print("Hello there. I'm a regular agent"); ?agent_id(AGENTID); ?path(PATH); .random(R); +my_testing(R); ?my_testing(R); .print("R: ",R); .abolish(agent_id(_609)); .abolish(path(_610)); ?sugar(SUGAR); ?metabolism(METABOLISM); ?vision(VISION); .print("Sugar ",SUGAR); .print("Metabolism ",METABOLISM); .print("Vision ",VISION); .abolish(sugar(_611)); .abolish(metabolism(_612)); .abolish(vision(_613)); .my_name(MYNAME); .concat("src/agt/list/",MYNAME,".asl",NAME); .save_agent(NAME,[start,say(hello)]); .print("Saved my information on file. Sending message to remove agent from simulation"); .send(killer_agent,tell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)); .send(killer_agent,untell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)).
@p__110[source(self),url("file:src/agt/default_agent.asl")] +free : (gsize(_614,W,H) & (jia.random(RX,(W-1)) & jia.random(RY,(H-1)))) <- .print("I am going to go near (",RX,",",RY,")"); !go_near(RX,RY).
...
@p__113[source(self),url("file:src/agt/default_agent.asl")] +!go_near(X,Y) : free <- -near(_615,_616); -last_dir(_617); !near(X,Y).
...
@p__119[source(self),url("file:src/agt/default_agent.asl")] +!next_step(X,Y) : not (pos(_618,_619)) <- !next_step(X,Y).
...
@pih1[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : .desire(near(_620,_621)) <- .print("Dropping near(_,_) desires and intentions to handle ",Gold); .drop_desire(near(_622,_623)); !init_handle(Gold).
@pih2[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : pos(X,Y) <- .print("Going for ",Gold); !!handle(Gold).
@pfunction[atomic,source(self),url("file:src/agt/default_agent.asl")] +!handle(gold(X,Y)) : not (free) <- .print("Handling ",gold(X,Y)," now."); !pos(X,Y); !ensure(pick,gold(X,Y)); ?depot(_624,DX,DY); !pos(DX,DY); !ensure(drop,0); .print("Finish handling ",gold(X,Y)); ?score(S); -+score((S+1)); .send(leader,tell,dropped); +initial_print1.
...
@p__126[source(self),url("file:src/agt/default_agent.asl")] +!ensure(pick,_625) : (pos(X,Y) & gold(X,Y)) <- pick; ?carrying_gold; -gold(X,Y).
@p__127[source(self),url("file:src/agt/default_agent.asl")] +!ensure(drop,_626) : (carrying_gold & (pos(X,Y) & depot(_627,X,Y))) <- drop.
...
@p__130[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : not (gold(_628,_629)) <- -+free.
@p__131[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : gold(_630,_631) <- .findall(gold(X,Y),gold(X,Y),LG); !calc_gold_distance(LG,LD); .length(LD,LLD); (LLD > 0); .print("Gold distances: ",LD,LLD); .min(LD,d(_632,NewG)); .print("Next gold is ",NewG); !!handle(NewG).
...
@p__135[source(self),url("file:src/agt/default_agent.asl")] +!calc_gold_distance([_633|R],RD) <- !calc_gold_distance(R,RD).
@p__136[source(self),url("file:src/agt/default_agent.asl")] +end_of_simulation(S,_634) <- .drop_all_desires; .abolish(gold(_635,_636)); .abolish(picked(_637)); -+free; .print("-- END ",S," --").
