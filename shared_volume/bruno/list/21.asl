// beliefs and rules
kqml::bel_no_source_self(NS::Content,Ans) :- (NS::Content[|LA] & (kqml::clear_source_self(LA,NLA) & ((Content =.. [F,T,_848]) & (Ans =.. [NS,F,T,NLA])))).
...
my_testing(0.07552622221440042).
...
focused(WksName,ArtName[artifact_type(Type)],ArtId) :- focusing(ArtId,ArtName,Type,_806,WksName,_807).
...
joined(WksName,WksId) :- joinedWsp(WksId,WksName,_808).
last_dir(down).
...
@p__137[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art([],_803).
...
@p__139[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art(art_env(W,"",_804),Try) <- .concat("/main/",W,FullW); joinWorkspace(FullW,_805); .print("joinned workspace ",FullW).
...
@pfunction2[atomic,source(self),url("file:src/agt/default_agent.asl")] +initial_print1 <- .print("Hello there. I'm a regular agent"); ?agent_id(AGENTID); ?path(PATH); .random(R); +my_testing(R); ?my_testing(R); .print("R: ",R); .abolish(agent_id(_809)); .abolish(path(_810)); ?sugar(SUGAR); ?metabolism(METABOLISM); ?vision(VISION); .print("Sugar ",SUGAR); .print("Metabolism ",METABOLISM); .print("Vision ",VISION); .abolish(sugar(_811)); .abolish(metabolism(_812)); .abolish(vision(_813)); .my_name(MYNAME); .concat("src/agt/list/",MYNAME,".asl",NAME); .save_agent(NAME,[start,say(hello)]); .print("Saved my information on file. Sending message to remove agent from simulation"); .send(killer_agent,tell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)); .send(killer_agent,untell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)).
@p__142[source(self),url("file:src/agt/default_agent.asl")] +free : (gsize(_814,W,H) & (jia.random(RX,(W-1)) & jia.random(RY,(H-1)))) <- .print("I am going to go near (",RX,",",RY,")"); !go_near(RX,RY).
...
@p__145[source(self),url("file:src/agt/default_agent.asl")] +!go_near(X,Y) : free <- -near(_815,_816); -last_dir(_817); !near(X,Y).
...
@p__151[source(self),url("file:src/agt/default_agent.asl")] +!next_step(X,Y) : not (pos(_818,_819)) <- !next_step(X,Y).
...
@pih1[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : .desire(near(_820,_821)) <- .print("Dropping near(_,_) desires and intentions to handle ",Gold); .drop_desire(near(_822,_823)); !init_handle(Gold).
@pih2[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : pos(X,Y) <- .print("Going for ",Gold); !!handle(Gold).
@pfunction[atomic,source(self),url("file:src/agt/default_agent.asl")] +!handle(gold(X,Y)) : not (free) <- .print("Handling ",gold(X,Y)," now."); !pos(X,Y); !ensure(pick,gold(X,Y)); ?depot(_824,DX,DY); !pos(DX,DY); !ensure(drop,0); .print("Finish handling ",gold(X,Y)); ?score(S); -+score((S+1)); .send(leader,tell,dropped); +initial_print1.
...
@p__158[source(self),url("file:src/agt/default_agent.asl")] +!ensure(pick,_825) : (pos(X,Y) & gold(X,Y)) <- pick; ?carrying_gold; -gold(X,Y).
@p__159[source(self),url("file:src/agt/default_agent.asl")] +!ensure(drop,_826) : (carrying_gold & (pos(X,Y) & depot(_827,X,Y))) <- drop.
...
@p__162[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : not (gold(_828,_829)) <- -+free.
@p__163[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : gold(_830,_831) <- .findall(gold(X,Y),gold(X,Y),LG); !calc_gold_distance(LG,LD); .length(LD,LLD); (LLD > 0); .print("Gold distances: ",LD,LLD); .min(LD,d(_832,NewG)); .print("Next gold is ",NewG); !!handle(NewG).
...
@p__167[source(self),url("file:src/agt/default_agent.asl")] +!calc_gold_distance([_833|R],RD) <- !calc_gold_distance(R,RD).
@p__168[source(self),url("file:src/agt/default_agent.asl")] +end_of_simulation(S,_834) <- .drop_all_desires; .abolish(gold(_835,_836)); .abolish(picked(_837)); -+free; .print("-- END ",S," --").
