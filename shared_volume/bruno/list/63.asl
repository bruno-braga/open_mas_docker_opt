// beliefs and rules
kqml::bel_no_source_self(NS::Content,Ans) :- (NS::Content[|LA] & (kqml::clear_source_self(LA,NLA) & ((Content =.. [F,T,_1781]) & (Ans =.. [NS,F,T,NLA])))).
...
focused(WksName,ArtName[artifact_type(Type)],ArtId) :- focusing(ArtId,ArtName,Type,_1739,WksName,_1740).
...
joined(WksName,WksId) :- joinedWsp(WksId,WksName,_1741).
last_dir(down).
...
@p__297[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art([],_1736).
...
@p__299[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art(art_env(W,"",_1737),Try) <- .concat("/main/",W,FullW); joinWorkspace(FullW,_1738); .print("joinned workspace ",FullW).
...
@pfunction2[atomic,source(self),url("file:src/agt/default_agent.asl")] +initial_print1 <- .print("Hello there. I'm a regular agent"); ?agent_id(AGENTID); ?path(PATH); .random(R); +my_testing(R); ?my_testing(R); .print("R: ",R); .abolish(agent_id(_1742)); .abolish(path(_1743)); ?sugar(SUGAR); ?metabolism(METABOLISM); ?vision(VISION); .print("Sugar ",SUGAR); .print("Metabolism ",METABOLISM); .print("Vision ",VISION); .abolish(sugar(_1744)); .abolish(metabolism(_1745)); .abolish(vision(_1746)); .my_name(MYNAME); .concat("src/agt/list/",MYNAME,".asl",NAME); .save_agent(NAME,[start,say(hello)]); .print("Saved my information on file. Sending message to remove agent from simulation"); .send(killer_agent,tell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)); .send(killer_agent,untell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)).
@p__302[source(self),url("file:src/agt/default_agent.asl")] +free : (gsize(_1747,W,H) & (jia.random(RX,(W-1)) & jia.random(RY,(H-1)))) <- .print("I am going to go near (",RX,",",RY,")"); !go_near(RX,RY).
...
@p__305[source(self),url("file:src/agt/default_agent.asl")] +!go_near(X,Y) : free <- -near(_1748,_1749); -last_dir(_1750); !near(X,Y).
...
@p__311[source(self),url("file:src/agt/default_agent.asl")] +!next_step(X,Y) : not (pos(_1751,_1752)) <- !next_step(X,Y).
...
@pih1[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : .desire(near(_1753,_1754)) <- .print("Dropping near(_,_) desires and intentions to handle ",Gold); .drop_desire(near(_1755,_1756)); !init_handle(Gold).
@pih2[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : pos(X,Y) <- .print("Going for ",Gold); !!handle(Gold).
@pfunction[atomic,source(self),url("file:src/agt/default_agent.asl")] +!handle(gold(X,Y)) : not (free) <- .print("Handling ",gold(X,Y)," now."); !pos(X,Y); !ensure(pick,gold(X,Y)); ?depot(_1757,DX,DY); !pos(DX,DY); !ensure(drop,0); .print("Finish handling ",gold(X,Y)); ?score(S); -+score((S+1)); .send(leader,tell,dropped); +initial_print1.
...
@p__318[source(self),url("file:src/agt/default_agent.asl")] +!ensure(pick,_1758) : (pos(X,Y) & gold(X,Y)) <- pick; ?carrying_gold; -gold(X,Y).
@p__319[source(self),url("file:src/agt/default_agent.asl")] +!ensure(drop,_1759) : (carrying_gold & (pos(X,Y) & depot(_1760,X,Y))) <- drop.
...
@p__322[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : not (gold(_1761,_1762)) <- -+free.
@p__323[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : gold(_1763,_1764) <- .findall(gold(X,Y),gold(X,Y),LG); !calc_gold_distance(LG,LD); .length(LD,LLD); (LLD > 0); .print("Gold distances: ",LD,LLD); .min(LD,d(_1765,NewG)); .print("Next gold is ",NewG); !!handle(NewG).
...
@p__327[source(self),url("file:src/agt/default_agent.asl")] +!calc_gold_distance([_1766|R],RD) <- !calc_gold_distance(R,RD).
@p__328[source(self),url("file:src/agt/default_agent.asl")] +end_of_simulation(S,_1767) <- .drop_all_desires; .abolish(gold(_1768,_1769)); .abolish(picked(_1770)); -+free; .print("-- END ",S," --").
